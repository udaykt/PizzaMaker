...
class PricingServiceTest {
    ...
    private OrderRequest request(SauceType sauceType,
    ...
    private OrderRequest carryout(List<ToppingSelection> toppings, PizzaSize size) {
        return request(SauceType.NONE, false, false, false, false, false, false, toppings, size, DeliveryMethod.CARRYOUT);
    }
    ...
    void standardSauceAndCheese_chargeFiftyCentsEach() {
        var req = request(SauceType.ROBUST_TOMATO, true, false, false, false, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
    ...
    void specialtySauces_costMoreThanStandard() {
        var marinara = request(SauceType.MARINARA, false, false, false, false, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
        var bbq = request(SauceType.BBQ, false, false, false, false, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
    ...
    void specialtyCheeses_costMoreThanMozzarella() {
        // feta is specialty ($1.00), cheddar is standard ($0.50)
        var withCheddar = request(SauceType.NONE, false, true, false, false, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
        var withFeta = request(SauceType.NONE, false, false, false, true, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
        assertThat(PricingService.computeTotal(withCheddar)).isEqualByComparingTo("12.50");
        // 12.00 + 1.00 (feta)
        assertThat(PricingService.computeTotal(withFeta)).isEqualByComparingTo("13.00");
    }

    @Test
    void multipleCheeses_stack() {
        // mozzarella (standard) + cheddar (standard) + feta (specialty)
        var req = request(SauceType.NONE, true, true, false, true, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
        // 12.00 + 0.50 (mozzarella) + 0.50 (cheddar) + 1.00 (feta)
        assertThat(PricingService.computeTotal(req)).isEqualByComparingTo("14.00");
    }
    ...
    void nullSauceType_addsNoCharge() {
        var req = request(null, false, false, false, false, false, false, List.of(), PizzaSize.M, DeliveryMethod.CARRYOUT);
    ...
    void fullyLoadedPizza_sumsCorrectly() {
        ...
        // bbq + mozzarella (std) + cheddar (std) + parmesanAsiago (specialty) + feta (specialty)
        var req = request(SauceType.BBQ, true, true, true, true, false, false, toppings, PizzaSize.L, DeliveryMethod.CARRYOUT);
        // 16 + 1.25 (bbq) + 0.50 (mozz) + 0.50 (cheddar) + 1.00 (parmAsiago) + 1.00 (feta) + 4*3.0 (toppings extra)
        assertThat(PricingService.computeTotal(req)).isEqualByComparingTo(BigDecimal.valueOf(16 + 1.25 + 0.5 + 0.5 + 1.0 + 1.0 + 12.0));
    }
    ...
    void delivery_addsFeeOnTopOfCarryoutPrice() {
        ...
        var deliveryReq = request(SauceType.NONE, false, false, false, false, false, false, List.of(), PizzaSize.M, DeliveryMethod.DELIVERY);
